...
from SR_Dataset_3 import SR_Dataset_3, ToTensor
import numpy as np
import matplotlib
...
class SR(nn.Module):
    ...
    def forward(self, image_t1, image_t2, image_t3):
        data_list = []
        ...
        for i in range(input[1].__len__()): # SISRNET INDIVIDUAL PRR IMAGE
...
def test(args, model, device, test_loader, epoch):
        ...
        for samples, t in test_loader:  
            ...
            for i in range(real.shape[0]):
                ...
                if i == args.batch_size - 1:
                    plot_images(t1, t2, t3, real_image, predicted_image, fusion_image, input_image, cnt, psnr_output/(cnt*args.batch_size), ssim_out/(cnt*args.batch_size), psnr_input/(cnt*args.batch_size), ssim_in/(cnt*args.batch_size)) 
    ...
    print('\nTest set: PSNR OUTPUT: ({:.2f} dB), PSNR INPUT: ({:.2f} dB), SSIM_OUTPUT: ({:.2f}, SSIM_INPUT: ({:.2f})\n'.format(
...
def plot_images(t1, t2, t3, target, predicted, fusion, input_imag, cnt, psnr_output, ssim_out, psnr_input, ssim_in):
    ...
    ep.plot_rgb(t2, rgb=(2, 1, 0), ax=f_ax3, title="Input 2")
    ep.plot_rgb(t3, rgb=(2, 1, 0), ax=f_ax5, title="Input 3")
...
def main():
    ...
    train_dataset = SR_Dataset_3(csv_file='PATH',
                              root_dir='PATH', transform=ToTensor(), stand=True, norm=False)

    test_dataset = SR_Dataset_3(csv_file='PATH',
                              root_dir='PATH', transform=ToTensor(), stand=True, norm=False)

    validation_dataset = SR_Dataset_3(csv_file='PATH',
                              root_dir='PATH', transform=ToTensor(), stand=True, norm=False)
