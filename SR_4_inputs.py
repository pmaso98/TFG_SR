...
from SR_Dataset_4 import SR_Dataset_4, ToTensor
import numpy as np
import matplotlib
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (samples, t) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, fusion, image_mean = model(image_t1, image_t2, image_t3, image_t4)
        loss_f = nn.MSELoss()
        ...
        for samples, t in test_loader:  
            ...
            for i in range(real.shape[0]):
                ...
                if i == args.batch_size - 1:
                    plot_images(t1, t2, t3, t4, real_image, predicted_image, fusion_image, input_image, cnt, psnr_output/(cnt*args.batch_size), ssim_out/(cnt*args.batch_size), psnr_input/(cnt*args.batch_size), ssim_in/(cnt*args.batch_size)) 
...
def plot_images(t1, t2, t3, t4, target, predicted, fusion, input_imag, cnt, psnr_output, ssim_out, psnr_input, ssim_in):
    ...
    ep.plot_rgb(t3, rgb=(2, 1, 0), ax=f_ax4, title="Input 3")
    ep.plot_rgb(t3, rgb=(2, 1, 0), ax=f_ax5, title="Input 4")
