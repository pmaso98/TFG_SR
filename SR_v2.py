...
class SR(nn.Module):
    ...
    def forward(self, image_t1, image_t2, image_t3, image_t4,
        ...
        for i in range(4): # forward por bandas
            ...
            band = input[:, :, i, :, :]
            print(i)
            print("entrada SISRNET"+str(band.shape))
            for j in range(band[1].__len__()): #SISRNET INDIVIDUAL POR IMAGEN

                x = torch.unsqueeze(band[:, j, :, :], 1)
                x = torch.unsqueeze(x, 2)
                print("S1"+str(x.shape))
                ...
                print("S5"+str(x.shape))

                x = self.conv2(x)
                x = self.instancenorm1(x)
                x = F.leaky_relu(x)

                print("S6"+str(x.shape))

                x = self.conv2(x)
                x = self.instancenorm1(x)
                x = F.leaky_relu(x)

                print("S7"+str(x.shape))

                x = self.conv2(x)
                x = self.instancenorm1(x)
                x = F.leaky_relu(x)

                print("S8"+str(x.shape))

                x = self.conv2(x)
                x = self.instancenorm1(x)
                x = F.leaky_relu(x)

                print("S9"+str(x.shape))
            ...
            print("Concat_SISRNET"+str(x.shape))

            # Mean of input images
            image_interpol = torch.cat(data_list, dim=1)
            print("image_interpol"+str(image_interpol.shape))
            image_mean = image_interpol.mean(dim=1)
            image_mean = torch.unsqueeze(image_mean, 1)
            print("image_mean_input"+str(image_mean.shape))

            # FusionNet
            f = self.conv3(x)
            f = self.instancenorm1(f)
            f = F.leaky_relu(f)

            print("F1"+str(f.shape))

            f = self.conv3(f)
            f = self.instancenorm1(f)
            f = F.leaky_relu(f)

            print("F2"+str(f.shape))

            f = self.conv3(f)
            f = self.instancenorm1(f)
            f = F.leaky_relu(f)

            print("F3"+str(f.shape))

            f = self.conv3(f)
            f = self.instancenorm1(f)
            f = F.leaky_relu(f)

            print("F4"+str(f.shape))

            f = self.conv4(f)
            f = self.instancenorm2(f)
            f = F.leaky_relu(f)

            print("Last_Fusion"+str(f.shape))

            images_FusionNet.append(f)

        img = torch.cat(images_FusionNet, dim=2)
        print("final_cat"+str(img.shape))
